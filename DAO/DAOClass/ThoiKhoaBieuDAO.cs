...
namespace Student_Management_System_CSharp_SGU2025.DAO
{
	public class ThoiKhoaBieuDAO
	{
		...
		public void InsertTemp(int semesterId, int weekNo, IEnumerable<AssignmentSlotDTO> slots)
		{
		...
		public List<AssignmentSlotDTO> GetWeek(int semesterId, int weekNo)
		{
			...
			var result = new List<AssignmentSlotDTO>();
			using (var conn = ConnectionDatabase.GetConnection())
			{
				conn.Open();
				using (var cmd = new MySqlCommand(sql, conn))
				{
					...
					using (var reader = cmd.ExecuteReader())
					{
						while (reader.Read())
						{
							result.Add(new AssignmentSlotDTO
							{
		...
		/// </summary>
		public List<AssignmentSlotDTO> GetWeekByClass(int semesterId, int weekNo, int maLop)
		{
		...
		public bool HasConflict(int semesterId, int weekNo, AssignmentSlotDTO slot)
		{
			const string sql = @"
				SELECT 1 FROM (
